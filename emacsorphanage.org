...
#+BEGIN_SRC emacs-lisp
  (epkg-with-org-header ("Package" "Mirror" "Melpa" "Reason")
    (--map (-let* ((name (cdr (assq 'name it)))
           ...
           (cl-sort (ghub-get "/orgs/emacsorphanage/repos" nil nil nil t)
                    #'string< :key (lambda (it) (cdr (assq 'name it))))))
#+END_SRC
#+RESULTS: overview
| Package (75)            | Mirror   | Melpa    | Reason                      |
|-------------------------+----------+----------+-----------------------------|
| anything-project        | orphaned |          |                             |
| apel                    | orphaned | github   | 6. many feature conflicts   |
| caml                    | orphaned | svn      | A. semi-manual svn import   |
| cg                      | orphaned | svn      | A. semi-manual svn import   |
| clang-format            | orphaned | svn      | A. semi-manual svn import   |
| color-theme             | orphaned | bzr      | 5. bzr not supported        |
| confluence              | orphaned | svn      | A. semi-manual svn import   |
| crontab-mode            | orphaned | orphaned | 3. upstream disappeared     |
| csv-nav                 | wiki     | wiki     |                             |
| dedicated               | orphaned | orphaned | 2. upstream has disappeared |
| dic-lookup-w3m          | orphaned | svn      | A. semi-manual svn import   |
...
| edb                     | orphaned |          | C. unmaintained, needed fix |
...
| project-local-variables | wiki     | wiki     |                             |
| projmake-mode           | orphaned | github   |                             |
| read-char-spec          | orphaned |          | 3. upstream unresponsive    |
| rfringe                 | wiki     | wiki     |                             |
| ruby-additional         | orphaned | svn      | A. semi-manual svn import   |
...
| sml-modeline            | orphaned | bzr      | 5. bzr not supported        |
...
| w3m                     | orphaned | cvs      | A. semi-manual cvs import   |
...
10. Like (5), but these ~svn~ and ~cvs~ repositories contain the full
    history, imported using ~git svn~ or ~git cvsimport~.

11. Multiple packages bundle this but none of them is the authorities
    upstream.  Attempts to get the various non-upstreams talking have
    failed so far.

12. Unmaintained for many years and need a feature fix.  I should
    probably use the (10) approach instead, but I haven't done so yet
    (?)  because this is not available from Melpa.
