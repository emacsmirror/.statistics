...
| chasen             | orphaned |          | 9. suffix not supported     |
...
| japanese-tokenizer | orphaned |          | 9. suffix not supported     |
...
9. These packages are still available from upstream at
   http://www.geocities.co.jp/SiliconValley-PaloAlto/7043/<name>/<name>.el.txt,
   but the import tools don't support the ~.txt~ suffix.  Also I believe
   that these files haven't been touched in over a decade and it seems
   unlikely that will change any time soon.
