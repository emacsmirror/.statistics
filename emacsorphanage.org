#+SETUPFILE: .orgconfig
#+NAME: setup
#+BEGIN_SRC emacs-lisp :results silent
  (require 'emir)

  (setq emir--orphans-cache
        (cl-sort (ghub-get "/orgs/emacsorphanage/repos" nil nil nil t)
                 #'string< :key (lambda (it) (cdr (assq 'name it)))))

  (defun emir--list-orphans (&rest only)
    (epkg-with-org-header ("Package" "Mirror" "Melpa")
      (--keep
       (-let* ((name (cdr (assq 'name it)))
               (pkg  (epkg name))
               (rcp  (melpa-get name))
               (reason (cadr (assoc name emir--orphanage-reason))))
         (and (or (not only)
                  (member reason only))
              (list name
                    (if pkg (closql--abbrev-class (eieio-object-class pkg)) "")
                    (if rcp
                        (if (string-prefix-p "emacsorphanage/" (oref rcp repo))
                            "orphaned"
                          (closql--abbrev-class (eieio-object-class rcp)))
                      "")
                    ;; (or reason "")
                    )))
       emir--orphans-cache)))
#+END_SRC
[[https://emacsmirror.net/stats][up]]

* Upstream has disappeared but there are dependents

The upstream repository or file download for these packages has
disappeared.

Or at least it wasn't available for a long time; in some cases the
upstream might be available again.  I do not check to make sure this
is not the case.  If you find such a package, then please let me know.

Of course there are many more packages whose upstream has disappeared,
then those listed here, many of which can be found on the Emacsattic.

Those unmaintained packages that made it into the orphanage instead of
just the attic, did so because other mirrored packages depend on them,
and/or because they are available from Melpa (I mirror all packages
that are available from there).

#+BEGIN_SRC emacs-lisp
  (emir--list-orphans "upstream disappeared")
#+END_SRC
#+RESULTS:
| Package (11) | Mirror   | Melpa    |
|--------------+----------+----------|
| crontab-mode | orphaned |          |
| dedicated    | orphaned | orphaned |
| ecasound     | orphaned |          |
| keydef       | orphaned | orphaned |
| miniedit     | orphaned | orphaned |
| osd          | orphaned |          |
| pager        | orphaned | orphaned |
| pointback    | orphaned | orphaned |
| smarty-mode  | orphaned | orphaned |
| tail         | orphaned |          |
| vala-mode    | orphaned | orphaned |

* Upstream is unresponsive but there are dependents

The upstream repositories for these packages still existed (or at
least they still existed when the packages were added here), but the
packages have issues that have to be fixed and the maintainers did not
respond when I contacted them.  I have patched these packages here in
the orphanage to fix these issues.

- Multiple packages bundle ~hyperspec~ but none of them is the
  authoritive upstream.  Attempts to get the various non-upstreams
  talking have failed so far.

#+BEGIN_SRC emacs-lisp
  (emir--list-orphans
   "upstream unresponsive"
   "multiple non-upstreams")
#+END_SRC
#+RESULTS:
| Package (11)   | Mirror   | Melpa    |
|----------------+----------+----------|
| doom           | orphaned | orphaned |
| edb            | orphaned |          |
| elisp-depend   | orphaned | orphaned |
| eshell-manual  | orphaned |          |
| filladapt      | orphaned |          |
| hyperspec      | orphaned |          |
| lively         | orphaned | orphaned |
| oberon         | orphaned | orphaned |
| org-bullets    | orphaned | orphaned |
| read-char-spec | orphaned |          |
| setnu          | orphaned |          |

* Wiki packages needed by non-wiki packages on Melpa

These packages are being distributed on the Emacswiki and other
packages that are not distributed on the Emacswiki depend on them.
Anyone can edit packages on the Emacswiki, which is a big security
risk, so eventually we want to remove those packages from Melpa.
Not importing these packages from the wiki anymore is a step into
that direction.

Also see Melpa issue [[https://github.com/melpa/melpa/issues/2342][#2342]].

#+BEGIN_SRC emacs-lisp
  (emir--list-orphans
   "wiki needed by non-wiki"
   "wiki multiple files")
#+END_SRC
#+RESULTS:
| Package (18)     | Mirror   | Melpa    |
|------------------+----------+----------|
| dirtree          | orphaned | orphaned |
| ert-expectations | orphaned | orphaned |
| findr            | orphaned | orphaned |
| hide-lines       | orphaned | orphaned |
| http-post-simple | orphaned | orphaned |
| key-chord        | orphaned | orphaned |
| levenshtein      | orphaned | orphaned |
| linkd            | orphaned |          |
| look-mode        | orphaned | orphaned |
| multi-term       | orphaned | orphaned |
| shell-command    | orphaned | orphaned |
| shell-history    | orphaned | orphaned |
| showtip          | orphaned | orphaned |
| sr-speedbar      | orphaned | orphaned |
| transpose-frame  | orphaned | orphaned |
| tree-mode        | orphaned | orphaned |
| w32-browser      | orphaned | orphaned |
| windata          | orphaned | orphaned |

* Imported from non-git repostiory

The tools used to maintain the Emacsmirror do not support ~bzr~, ~cvs~,
~darcs~, ~fossil~, or importing from a tarball.  So I manually import the
upstream repositories/tarballs into a ~git~ repository on the orphanage
and then import the resulting ~git~ repositories as if that were the
actual upstream.

I do not regularly update these repositories.  If you find that they
are outdated, then please let me know.  However many of these package
repositories have not seen a commit in a decade.

#+BEGIN_SRC emacs-lisp
  (emir--list-orphans
   "semi-manual cvs import"
   "semi-manual svn import"
   "bzr not supported"
   "cvs not supported"
   "fossil not supported")
#+END_SRC
#+RESULTS:
| Package (16)    | Mirror   | Melpa    |
|-----------------+----------+----------|
| caml            | orphaned | svn      |
| cg              | orphaned | svn      |
| clang-format    | orphaned | svn      |
| color-theme     | orphaned | bzr      |
| confluence      | orphaned | svn      |
| dic-lookup-w3m  | orphaned | svn      |
| dsvn            | orphaned | svn      |
| helm-ls-svn     | orphaned | svn      |
| nxhtml          | orphaned |          |
| ruby-additional | orphaned | svn      |
| sml-modeline    | orphaned | bzr      |
| swbuff          | orphaned | orphaned |
| text-translator | orphaned |          |
| vc-fossil       | orphaned | orphaned |
| vm              | orphaned |          |
| w3m             | orphaned | cvs      |

* Imported from tarball
#+BEGIN_SRC emacs-lisp
  (emir--list-orphans "manual tarball import")
#+END_SRC
#+RESULTS:
| Package (2) | Mirror   | Melpa    |
|-------------+----------+----------|
| picolisp    | orphaned |          |
| session     | orphaned | orphaned |

* The maintainer is dead

Sadly there is one package where "orphan" has a more literal meaning
than what I had in mind when I created the Emacs Orphanage.  RIP
François.

#+BEGIN_SRC emacs-lisp
  (emir--list-orphans "maintainer is dead")
#+END_SRC
#+RESULTS:
| Package (1) | Mirror   | Melpa    |
|-------------+----------+----------|
| org-grep    | orphaned | orphaned |

* New
#+BEGIN_SRC emacs-lisp
  (emir--list-orphans nil)
#+END_SRC
#+RESULTS:

