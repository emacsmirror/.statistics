...
| gradle                          | github   | project-root             | shelved soft                                      |
...
#+begin_src emacs-lisp
  (epkg-with-org-header (3 "Package" "Reason" "Suspended" "M" "Url")
    (mapcar (pcase-lambda (`(,name ,args ,url))
              (list name
                    (cadr (assoc name emir--minority-reason))
                    (cadr (assoc name emir-suspended-packages))
                    ;; (mapconcat #'identity args " ")
                    (and (melpa-get name) "✓")
                    url))
            (cl-sort (epkgs [name upstream-tree url] 'epkg-minority-package-p)
                     #'string< :key #'caddr)))
#+end_src
...
#+end_src
#+results: no-mainlib
| Package (21)          | Class    | M | Library                         | Reason                     |
|-----------------------+----------+---+---------------------------------+----------------------------|
...
#+name: set-branch
#+begin_src emacs-lisp
(epkg-with-org-header ("Package" "Branch" "Class" "Repo")
...
#+end_src
#+results: set-branch
| Package (8)        | Branch                     | Class     | Repo                                             |
|--------------------+----------------------------+-----------+--------------------------------------------------|
| async-backup       | development                | git       | [[https://tildegit.org/contrapunctus/async-backup]]  |
| chronometrist      | dev                        | git       | [[https://tildegit.org/contrapunctus/chronometrist]] |
| elpa-admin         | elpa-admin                 | git       |                                                  |
| jabber             | production                 | git       | [[https://codeberg.org/emacs-jabber/emacs-jabber]]   |
| shell-command+     | shell-command+             | sourcehut | [[https://git.sr.ht/~zge/bang][zge/bang]]                                         |
| sxiv               | dev                        | git       | [[https://tildegit.org/contrapunctus/sxiv]]          |
...
- chronometrist (by contrapunctus)
- jabber (by contrapunctus)
