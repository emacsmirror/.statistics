...
| License           | Count | Percent |
|-------------------+-------+---------|
| GPL-3.0-or-later  |  4160 |      66 |
| GPL-2.0-or-later  |  1163 |      19 |
| MIT               |   353 |       6 |
| GPL-3.0-only      |   186 |       3 |
| BSD-2-Clause      |    84 |       1 |
| BSD-3-Clause      |    65 |       1 |
...
| clhs           | gitlab |          |
| dired-single   | github |          |
| nyan-prompt    | github | COPYLOVE |
| password-vault | github | COPYLOVE |

- clhs: https://gitlab.com/sam-s/clhs/-/merge_requests/1
- dired-single: https://github.com/crocket/dired-single/issues/1
- nyan-prompt, password-vault: https://github.com/PuercoPop/password-vault/issues/4

** Unknown licenses
#+NAME: mirror-unknown-licenses
#+BEGIN_SRC emacs-lisp
  (epkg-with-org-header ("License")
    (cl-set-difference (delete '(nil)
                               (epkg-sql [:select :distinct [license]
                                          :from packages
                                          :where (not (= class 'shelved))
                                          :order-by (asc license)]))
                       (mapcar #'list elx-spdx-license-identifiers)
                       :test #'equal))
#+END_SRC
#+RESULTS: mirror-unknown-licenses
| License (3)   |
|---------------|
| COPYLOVE      |
| as-is         |
